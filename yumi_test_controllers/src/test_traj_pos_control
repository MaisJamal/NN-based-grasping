...
void left_pos_controller_callback(const ros::SingleSubscriberPublisher& pub)
{
...
void right_pos_controller_callback(const ros::SingleSubscriberPublisher& pub)
{
	ROS_INFO("New subscriber!");
    numSubscribersConnected++;
}
...
int main( int argc, char* argv[] )
{
	...
	ros::init(argc, argv, "test_joint_pos_control");
	ros::NodeHandle nh;


    ROS_INFO("test_join_pos_control node started!");
    ...
    left_pub = nh.advertise<trajectory_msgs::JointTrajectory>("/yumi/joint_traj_pos_controller_l/command", 100, left_pos_controller_callback);
	right_pub = nh.advertise<trajectory_msgs::JointTrajectory>("/yumi/joint_traj_pos_controller_r/command", 100, right_pos_controller_callback);
